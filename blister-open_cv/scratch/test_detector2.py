...
output_path = r"c:\Users\hampl\OneDrive\ドキュメント\GitHub\Blister-bot-main-\blister-detector\outputs\test_result2.jpg"
...
# 1. Packet Masking (Segment blister pack from background)
hsv = cv2.cvtColor(proc_img, cv2.COLOR_BGR2HSV)
gray = cv2.cvtColor(proc_img, cv2.COLOR_BGR2GRAY)

# Packet mask using adaptive threshold + saturation/value analysis
sat = hsv[:, :, 1]
val = hsv[:, :, 2]

# Find packet bounds
_, packet_mask = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
kernel_pkt = cv2.getStructuringElement(cv2.MORPH_RECT, (15, 15))
packet_mask = cv2.morphologyEx(packet_mask, cv2.MORPH_CLOSE, kernel_pkt)

# Find largest contour (the blister strip)
...
# 2. Pill/Blister Circle Detection
...
for (cx, cy, r) in filtered_circles:
    ...
    orig_r = int(r / scale)
    # Draw clean strict red boundary
    cv2.circle(output_img, (orig_cx, orig_cy), orig_r, (0, 0, 255), 6)

print(f"Detected {len(filtered_circles)} strict pill boundaries inside blister packet.")
